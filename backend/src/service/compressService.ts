...
export const compressFilesService = async (files: string[]): Promise<string> => {
  ...
  const zipFilePath = path.join(OUTPUT_DIR, await getUniqueFileName(OUTPUT_DIR, 'compressed_files.zip'));
